...
-- Lock down the SECURITY DEFINER maintenance function. It runs as its owner
-- (postgres) and bypasses RLS, so it must NOT be reachable via the public
-- PostgREST API: without this revoke, Postgres grants EXECUTE to PUBLIC and the
-- anon key (shipped in the app) can call POST /rest/v1/rpc/telemetry_rollup_and_prune
-- and, e.g., pass a negative retention to delete every raw row. pg_cron runs the
-- job as the owner and manual runs use service_role, so no API role needs EXECUTE.
-- NOTE: `supabase db diff` does not reliably capture function ACLs, so this revoke
-- is also carried in a hand-written migration (…_telemetry_rollup_revoke.sql).
